...
The aim is to calculate the efficiency gain for sampling each distribution on the real line.
...
The efficiency _gain_ is the calculated as the ratio (unbounded over bounded) of the efficiencies.
...
if (runLoop) {
  ...
  ## write.table(efficiencyGain, file=here::here("data/efficiencyGain.csv"), sep=";", row.names = FALSE)
  ## save(efficiencyGain, file=here::here("data/efficiencyGain.Rdata"))
} else {
  data(efficiencyGain)
}
```

Finally, some boxplots of the efficiency gain per distribution, annotated with the with mean and 95% CIs.
