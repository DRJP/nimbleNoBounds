...
cran_downloads(packages="nimbleChangeOfVariables", when="last-week")


# How many overall downloads
mls <- cran_downloads(packages="nimbleChangeOfVariables", from = "2021-11-11", to = Sys.Date()-1)
mls
sum(mls[,2])

# Plot
gr0 <- ggplot(mls, aes(date, count)) + geom_line(colour = "red",size=1)
gr0 + xlab("Time") + ylab("Nr. of downloads") + labs(title = paste0("nimbleChangeOfVariables daily downloads ", Sys.Date()-1))
...
gr1 + xlab("Time") + ylab("Cumulative downloads") +
  labs(title = paste0("nimbleChangeOfVariables downloads: ", tStart, " to ", tStop))
